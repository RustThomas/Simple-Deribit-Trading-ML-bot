...
```Fetch Data from Deribit through the API -> Train Model with k last Data up to current time t -> Use model to predict price Value at t+1 -> Set up entry and exit orders (given none is currently open) ```
...
Significant tuning and changes are likely needed, including model, data and covariates. Consider user defined metrics, especially related to supports, resistances, maybe RSI and be aware of Regime Shifts. The trading logic is very primitive and needs improvement as well as refined trade exit logic. Model re-training may be skipped for a period of time or the resolution changed if it overlaps with the next one.
